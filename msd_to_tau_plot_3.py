...
def calculate_MSD(positions, tau_max=0):
    # Determine the maximum lag time based on the positions list length and tau_max
    if tau_max != 0:
        tau_max = int(len(positions) / tau_max)
    else:
        tau_max = len(positions) - 1

    # Generate a list of lag times
    tau = []
    for i in range(1, tau_max + 1):
        tau.append(i)

    # Calculate the Mean Squared Displacement (MSD) for each lag time
    MSD = []
    for i in tau:
        # Calculate the displacement for each pair of positions separated by the lag time i
        displacements = []
        for j in range(len(positions) - i):
            # Calculate the squared displacement for the current pair of positions
            squared_displacement = 0
            for k in range(len(positions[j])):
                squared_displacement += (positions[j + i][k] - positions[j][k]) ** 2
            # Take the square root of the squared displacement to get the actual displacement
            displacement = math.sqrt(squared_displacement)
            displacements.append(displacement)

        # Calculate the average displacement for the current lag time
        if displacements:
            average_displacement = sum(displacements) / len(displacements)
        else:
            average_displacement = float('nan')
        MSD.append(average_displacement)

    # Return the list of lag times and the corresponding MSD values
    return tau, MSD
...
def main():
    ...
    for directory in directories:
        ...
        write_image_to_directory(img, directory, 'msd_vs_tau_plot_3.png')
